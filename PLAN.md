...
- **Amazon sample + resilience (committed)** `eda/amazon_eda.ipynb` + `eda/SUMMARY.md` (rated-only, stratified ≤100/category → 28,242 products / 296 categories), `scripts/sample_amazon.py`, `data/amazon_sample.csv`; `products.py` now embeds/upserts in resilient 50-item chunks with bounded retries so multi-thousand-product backfills survive transient Mesh/Pinecone errors.
- **Storefront redesign (committed)** Amazon-pattern UI with a distinct teal/emerald theme: top utility bar + centered category search, sub-nav of top categories, dense product cards + star ratings, personalized homepage hero + "Recommended for you" rail, product detail buy-box, breadcrumbs. All tracking contracts preserved; `time_spent` tracking added on page hide.
